...
many values, such as
\[
J(\theta)=\frac{1}{n}\sum_{i=1}^{n}f_{i}(\theta)
\]
...
chosen uniformly at random from $\{1,\ldots,n\}$. The approximation
is poor, but we will show it is unbiased.

\noindent In practical implementations, the $n$ data points are \textbf{randomly
...
chosen uniformly at random from $\{1,\ldots,n\}$, is an \textbf{unbiased
